...
interface IProps {
  item: TestCase;
  delete_function: (({ id }: { id: number }) => Promise<void>) | undefined;
}
...
export const TestCaseListItem = ({
  ...
  const transformed_expected_array = get2dimensional(
    item.expected_value,
    item.m,
  );
  ...
  return (
    <Paper elevation={8} className="p-3 my-3">
      {item.is_successful === true && (
        <div className="text-green-500 text-2xl flex items-center justify-end">
          Успешно <DoneIcon />
        </div>
      )}
      {item.is_successful === false && (
        <div className="text-red-500 text-2xl flex items-center justify-end">
          Неуспешно <CloseIcon />
        </div>
      )}
      <div className="grid grid-cols-9 gap-4 items-start">
        ...
        {transformed_expected_array &&
          transformed_expected_array.length > 0 && (
            <div className="col-span-2">
              ...
              <div
                ...
                style={{
                  gridTemplateColumns: `repeat(${transformed_expected_array[0].length}, minmax(0, 1fr))`,
                }}
              >
        ...
        {transformed_output_array && transformed_output_array.length > 0 && (
          <div className="col-span-2">
            ...
            <div
              className="grid gap-1"
              style={{
                gridTemplateColumns: `repeat(${transformed_output_array[0].length}, minmax(0, 1fr))`,
              }}
            >
      ...
      <div className="flex flex-row gap-2">
        <Link className="" href={`/test_case/${item.id}`}>
          <Button variant="outlined" className="my-4">
            <ModeEditIcon />
          </Button>
        </Link>
        {delete_function && contextController.user?.is_admin && (
          <Button
            variant="outlined"
            className="my-4"
