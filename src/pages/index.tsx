...
const IndexPage: NextPageWithLayout = () => {
  ...
  const { data: test_case_list_query } = trpc.testCaseRouter.list.useQuery(
    undefined,
    {
      refetchOnMount: true,
  ...
  const handleDeleteTestCase = async ({ id }: { id: number }) => {
    ...
    set_test_cases_list([
      ...(test_cases_list || []).filter((i) => i.id !== id),
    ]);
  };
  ...
  return (
    <>
      ...
      <TestCaseUpsertField
        item={undefined}
        handle_update_function={handleNewTestCase}
      />
      {test_cases_list.map((item) => {
        return (
          <TestCaseListItem
            delete_function={handleDeleteTestCase}
            key={item.id}
            item={item}
          />
        );
